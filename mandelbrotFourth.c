...
int main(){
    ...
    for(int x = -halfWidth; x < halfWidth; x++){
        for(int y = -halfHeight; y < halfHeight; y++){
            ...
            while(count < totalIterations){
                ...
                //this is done: (a+bi)^4 = (a^4-6a^2b^2+b^4)+(4a^3b-4ab^3)i
                float realTerm = a*a*a*a - 6*a*a*b*b + b*b*b*b; //real component
                float complexTerm = 4*a*a*a*b - 4*a*b*b*b; //complex component 'coeffiecnt' <- correct usage?
     ...
     */
    FILE* pgmimg = fopen("mandelbrotFourth.pgm", "w");
    fprintf(pgmimg, "P2\n");
    fprintf(pgmimg, "%d %d\n", width, height);
    fprintf(pgmimg, "255\n");
