...
int main(){
    ...
    for(int x = -halfWidth; x < halfWidth; x++){
        for(int y = -halfHeight; y < halfHeight; y++){
            ...
            while(count < totalIterations){
                ...
                float realTerm = pow(a, 5)-10*pow(a, 3)*b*b+5*a*pow(b, 4); //real component
                float complexTerm = 4*pow(a, 4)*b-10*a*a*pow(b, 3)+pow(b, 5); //complex component 'coeffiecnt' <- correct usage?
     ...
     */
    FILE* pgmimg = fopen("mandelbrotFifth.pgm", "w");
    fprintf(pgmimg, "P2\n");
    fprintf(pgmimg, "%d %d\n", width, height);
    fprintf(pgmimg, "255\n");
